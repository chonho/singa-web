...
const vector<int> Tester::SliceParams(const vector<Param*>& params) {
...
void Tester::SetupWorkerServer(
  ...
  //CLEEauto slices = SliceParams(net->params());

  std::unordered_map<int, shared_ptr<NeuralNet>> grp_net;
  //int first_grp = classifiers.size() ? classifiers.at(0)->grp_id() : -1;
  for (auto worker : classifiers) {
	...
	if(cluster->share_memory())
      ...
      for (auto layer : grp_net[grp_id]->layers()) {
        bool local = layer->partition_id() >= classifiers.front()->id()
          && layer->partition_id() <= classifiers.back()->id();
...
vector<Server*> Tester::CreateServers(int nthreads, const JobProto& job) {
  auto cluster = Cluster::Get();
...
void Tester::CreateWorkers(int numClassifiers, const JobProto& job, vector<Classifier*>& classifiers) {
  ...
  */
  ...
  for (int gid = gstart; gid < gend; gid++) {
    for (int wid = wstart; wid < wend; wid++) {
      ...
      classifier->Init(nthreads++, gid, wid);
  ...
  */
  ...
  int nAsync = 0;
  int nThreads = 0;
  for (int wid = 0; wid < numClassifiers; wid++) {
      auto *classifier = Classifier::Create(job);
      classifier->Init(nThreads++, nAsync++, wid);
      classifiers.push_back(classifier);
  }
}

void Tester::Resume(JobProto* jobConf) {
  tinydir_dir dir;
...
//CLEEvoid Tester::Start(bool resume, const SingaProto& singaConf, JobProto* job) {
void Tester::Start(bool resume, const SingaProto& singaConf, JobProto* job, vector<Classifier*>* classifiers, int numClassifiers) {
  ...
  int nthreads = 1;
  nthreads = numClassifiers;
  //const vector<Classifier*> classifiers = CreateWorkers(nthreads, *job);
  CreateWorkers(nthreads, *job, *classifiers);
  //CLEEnthreads += classifiers.size();
  nthreads += classifiers->size();
  const vector<Server*> servers = CreateServers(nthreads, *job);
  //CLEESetupWorkerServer(*job, classifiers, servers);
  SetupWorkerServer(*job, *classifiers, servers);
