...
Classifier* Classifier::Create(const JobProto& proto) {
  auto factory = Singleton<Factory<singa::Classifier>>::Instance();
  Classifier* worker = nullptr;
...
void Classifier::Init(int thread_id, int grp_id, int id) {
...
void Classifier::InitLocalParams() {
...
void Classifier::Checkpoint(int step, shared_ptr<NeuralNet> net) {
  if (grp_id_ == 0) {
    BlobProtos bps;
...
void Classifier::ConnectStub(int grp, int id, Dealer* dealer, EntityType entity) {
...
int Classifier::Put(Param* param, int step) {
  Msg* msg=new Msg(Addr(grp_id_, id_, kClassifierParam), Addr(-1, -1, kStub));
...
int Classifier::Get(Param* param, int step) {
...
int Classifier::Update(Param* param, int step) {
...
int Classifier::CollectAll(shared_ptr<NeuralNet> net, int step) {
...
void Classifier::SendBlobs(
    bool data, bool grad, BridgeLayer* layer, shared_ptr<NeuralNet> net) {
  auto dst=layer->dstlayers().at(0);
  Msg *msg=new Msg();
  msg->set_src(Addr(grp_id_, id_, kClassifierLayer));
  msg->set_dst(Addr(grp_id_, dst->partition_id(), kClassifierLayer));
...
bool Classifier::CheckpointNow(int step) const {
  return (grp_id_ == 0
...
bool Classifier::TestNow(const int step) const {
  return (grp_id_ == 0
...
bool Classifier::ValidateNow(const int step) const {
  return (grp_id_ == 0
...
/* CLEE
void BPClassifier::TestOneBatch(int step, Phase phase,
...
/*CLEE
void CDClassifier::TestOneBatch(int step, Phase phase,
    shared_ptr<NeuralNet> net, Metric* perf) {
  auto& layers = net->layers();
  for (auto layer : layers)
    layer->ComputeFeature(kPositive | kForward, perf);
